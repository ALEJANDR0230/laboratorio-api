"""
servidor.py — API Flask para procesar PDFs de laboratorio
=========================================================
Basado 100% en leer_pdf.py original.
Solo se agrega la capa Flask al final — toda la lógica es idéntica.

Deploy: Railway / Render / Fly.io conectando repo de GitHub.
Instalar: pip install flask flask-cors pdfplumber pypdf gunicorn
Correr local: python servidor.py
"""
...
try:
    ...
    print("Instalando pdfplumber..."); _pip("pdfplumber"); import pdfplumber

try:
    from pypdf import PdfReader
except ImportError:
    print("Instalando pypdf..."); _pip("pypdf"); from pypdf import PdfReader
...
def extraer_texto(ruta):
    """Extrae texto de cada página. Siempre devuelve lista, nunca falla."""
    paginas = []
    try:
        ...
        try:
            ...
            raise RuntimeError(
                f"No se pudo leer el PDF:\n   pdfplumber: {e_plumber}\n   pypdf: {e_pypdf}"
            )
    return paginas


# ═════════════════════════════════════════════════════════════════════════════
#  PASO 2 — EXTRAER DATOS DEL PACIENTE (flexible, multi-formato)
# ═════════════════════════════════════════════════════════════════════════════

# Cada entrada: (campo, [lista de patrones alternativos])
# Se prueba cada patrón en orden hasta encontrar uno que funcione.
PATRONES_PACIENTE = {
    "folio": [
        r"Folio[:\s#]+([A-Z0-9\-]+)",
    ...
    "paciente_nombre": [
        r"Paciente[:\s]+\d+\s*[-–]\s*([A-ZÁÉÍÓÚÑ][A-ZÁÉÍÓÚÑa-záéíóúñ ]{3,50})(?:\n|Fecha|Sexo|M[eé]dico|$)",
        r"Paciente[:\s]+([A-ZÁÉÍÓÚÑ][A-ZÁÉÍÓÚÑa-záéíóúñ ,\.]+?)(?:\n|Fecha|Sexo|M[eé]dico)",
        r"Nombre[:\s]+([A-ZÁÉÍÓÚÑ][A-ZÁÉÍÓÚÑa-záéíóúñ ,\.]+?)(?:\n|Fecha|Sexo)",
        r"(?:Sr\.|Sra\.|Dr\.|Dra\.)?\s*([A-ZÁÉÍÓÚÑ]{2,}(?:\s+[A-ZÁÉÍÓÚÑ]{2,}){1,4})\s*\n",
    ],
    "sexo": [
...
# ═════════════════════════════════════════════════════════════════════════════
#  PASO 3 — DETECTOR UNIVERSAL DE ANALITOS
#  Estrategia: buscar líneas con patrón  NOMBRE  NUMERO  UNIDAD  [REF]
# ═════════════════════════════════════════════════════════════════════════════

# Unidades conocidas en laboratorio clínico
UNIDADES = (
...
# Palabras a ignorar (no son analitos)
PALABRAS_RUIDO = {
    ...
    # palabras inglesas de rangos/referencias
    "optimal","moderate","high","low","desirable","target","category","risk",
    "based","patients","treatment","depending","above","below","within","outside",
    # palabras que indican texto de referencia, no nombre de analito
    "menos","mayor","menor","de","del","al","con","para","que","niveles",
}

# Prefijos de línea que indican texto explicativo, no un analito real
PREFIJOS_RUIDO = (
...
# Patrón principal: NOMBRE_ANALITO  VALOR_NUMERICO  UNIDAD
...
# Patrón alternativo: NOMBRE  VALOR  (sin unidad conocida pero con referencia)
...
def limpiar_nombre(nombre):
    """Normaliza el nombre del analito para usarlo como clave de BD."""
    nombre = nombre.strip()
...
def parsear_valor(raw):
    """Convierte '< 5.3' o '10,5' a float limpio."""
    limpio = raw.replace(",", ".").replace(" ", "")
    prefijo = ""
    if limpio.startswith("<"):
        prefijo = "<"; limpio = limpio[1:]
...
def calcular_estatus(valor_num, ref_low, ref_high, prefijo):
    """Determina si el valor está dentro del rango de referencia."""
...
def extraer_analitos(texto):
    """
    Detector universal: recorre cada línea buscando analitos.
    Funciona con cualquier laboratorio sin importar el formato.
    """
    resultados = []
    nombres_vistos = set()
    lineas = texto.split("\n")
    ...
    for linea in lineas:
        ...
        # Pre-filtro: descartar líneas que son claramente rangos de referencia
        ...
        # Intento 1: patrón con unidad conocida
        ...
        # Intento 2: patrón sin unidad pero con rango
        if analito is None:
        ...
        # Normalizar nombre con sinónimos y deduplicar
        if analito:
...
# Sinónimos: mapea variantes en inglés/español al nombre canónico español
SINONIMOS_ANALITO = {
...
def extraer_analitos_fallback(texto, ya_encontrados):
    """
    Patrones adicionales para analitos que se escriben de forma no estándar.
    Solo agrega los que no fueron detectados por el método principal.
    """
    import unicodedata
    def _norm(s):
...
# ═════════════════════════════════════════════════════════════════════════════
#  PASO 4 — DETECTAR SECCIÓN / TIPO DE ESTUDIO
# ═════════════════════════════════════════════════════════════════════════════
...
def detectar_secciones(texto):
    ...
    return list(dict.fromkeys(encontradas))  # sin duplicados, mantiene orden


# ═════════════════════════════════════════════════════════════════════════════
#  PASO 5 — ENSAMBLAR REGISTRO FINAL
# ═════════════════════════════════════════════════════════════════════════════
...
# ═════════════════════════════════════════════════════════════════════════════
#  FLASK  ←  única parte nueva respecto a leer_pdf.py original
# ═════════════════════════════════════════════════════════════════════════════

@app.route("/", methods=["GET"])
def health():
    return jsonify({"status": "ok", "mensaje": "API de laboratorio activa"}), 200
...
def procesar_pdf():
    ...
    try:
        # ── misma secuencia que main() en leer_pdf.py ──────────────────────
...
if __name__ == "__main__":
    ...
    print(f"   Salud:    GET  http://localhost:{port}/")
    print(f"   Endpoint: POST http://localhost:{port}/procesar-pdf\n")
